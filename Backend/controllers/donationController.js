...
/**
 * Create a donation response (donor responds to a blood request)
 */
  ...
  try {
    ...
    if (bloodRequest.length === 0) {
      return res.status(404).json({
        success: false,
        message:
          'Blood request not found or has expired',
      });
    }
    ...
    // Create donation response with 'pending' status
...
export const getDonationResponsesByRequestId = async (req, res) => {
  try {
    ...
    if (
      bloodRequest[0].requester_id !== req.user.user_id &&
      !req.user.is_admin
    ) {
...
export const updateDonationResponse = async (req, res) => {
  try {
    ...
    // Requesters cannot cancel other people's responses
    if (
      isRequester &&
      !isAdmin &&
      status === 'cancelled' &&
      response.donor_id !== req.user.user_id
    ) {
      return res.status(403).json({
        success: false,
...
export const getMyDonationResponses = async (req, res) => {
  try {
    ...
    console.error('Error getting user donation responses:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
    });
...
export const deleteDonationResponse = async (req, res) => {
  try {
    ...
    if (
      donationResponse[0].donor_id !== req.user.user_id &&
      !req.user.is_admin
    ) {
      return res.status(403).json({
        success: false,
