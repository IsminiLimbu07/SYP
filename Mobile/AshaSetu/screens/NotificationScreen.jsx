...
const openInMaps = async (lat, lng, label) => {
  const encodedLabel = encodeURIComponent(label || 'Safe Location');
  
  // Google Maps app deep links — work on both platforms
  const googleMapsApp = `comgooglemaps://?q=${lat},${lng}&zoom=15`;
  const googleMapsBrowser = `https://www.google.com/maps/search/?api=1&query=${lat},${lng}&query_place_id=${encodedLabel}`;
  ...
  try {
    ...
    const googleSupported = await Linking.canOpenURL(googleMapsApp);
    if (googleSupported) {
      await Linking.openURL(googleMapsApp);
      return;
    }

    // 2. Try native maps (Apple Maps on iOS, default maps on Android)
    ...
    // 3. Final fallback — open Google Maps in browser
    await Linking.openURL(googleMapsBrowser);
