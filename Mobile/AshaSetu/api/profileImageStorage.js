...
const uploadProfilePictureToServer = async (uri) => {
  try {
    ...
    const uploadUrl = apiConfig.ENDPOINTS.AUTH.UPLOAD_PROFILE_PICTURE;
    ...
    console.log('📋 Raw response:', {
      ...
      first200Chars: responseText.slice(0, 200),
    });
    ...
    try {
      ...
      console.error('❌ Upload Error: invalid JSON from server', {
        status: response.status,
        body: responseText.slice(0, 500),
        parseError: parseError.message,
      });
      throw new Error(
        `Upload failed: server returned non-JSON response (status ${response.status}).` +
